"""
api.py — FastAPI Application Layer  (Week 7 Day 1 — Logging & Observability)
=============================================================================
Extends Day 2 with full production-grade observability:

NEW in Week 7 Day 1:
  - observability.init_logging() called at module-level startup → rotating log file
  - new_trace_id() generates a UUID4 per /chat request, injected into graph_input
    and all request-level log entries
  - trace_id returned in ChatResponse so UI can display it for support correlation
  - Middleware logs now include trace_id (extracted from response headers)
  - GET /logs endpoint — returns last N lines from rotating log file (auth-protected)
  - Ingest endpoint passes trace_id into vector_manager.ingest_document()
  - All error handlers log full traceback as structured field
"""
...
# ==============================================================================
app = FastAPI(
    title="SwiftShip Logistics Support — RAG API v3",
    description=(
        "Customer Support AI agent for SwiftShip Logistics. "
        "Week 7 Day 1: Industry-standard structured logging & observability. "
        "Includes rotating log files, per-request trace IDs, token tracking, "
        "latency measurement at every node, and a /logs endpoint."
    ),
    version="3.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def get_metrics(_: None = Depends(verify_api_key)):
    ...
    return {
        ...
        "total_tokens_in":      _metrics["total_tokens_in"],       # ★ NEW
        "total_tokens_out":     _metrics["total_tokens_out"],      # ★ NEW
        "avg_latency_ms":       avg_latency,
        "active_sessions":      memory_store.active_count(),
        "started_at":           _metrics["started_at"],
        "log_dir":              LOG_DIR,                           # ★ NEW
        "log_level":            LOG_LEVEL,                         # ★ NEW
    }
...
async def chat(request: ChatRequest):
    ...
    graph_input = {
        ...
        "failure_mode":       None,
        "error_detail":       None,
        "trace_id":           trace_id,    # ★ NEW — threaded through all nodes
    }

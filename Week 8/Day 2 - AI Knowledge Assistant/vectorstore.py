"""
vectorstore.py — ChromaDB Storage Layer  (Week 8 Day 2 — AI Knowledge Assistant)
==================================================================================
General-purpose knowledge base — no domain category taxonomy.

Inherited from Week 8 Day 1:
  - Adaptive chunking (dense vs prose document detection)
  - Duplicate detection via MD5 file hash
  - Timeout + circuit breaker wrapping on retrieve()
  - Full observability via log_retrieval()

NEW in Week 8 Day 2:
  - Removed category filter requirement (general-purpose system)
  - search_kb(query, top_k) — standalone KB search without the chat pipeline
  - get_document_by_hash(file_hash) — look up a specific document's metadata
"""
...
class VectorStoreManager:
    """
    Manages ChromaDB as a general-purpose document knowledge base.
    Thread-safe for concurrent ingest + retrieval from the API layer.
    """
    ...
    def ingest_document(
        ...
        Args:
            ...
            source_name: Original filename (stored as metadata).
            page_count:  Number of pages (from ingestor).
            file_type:   "pdf" or "txt".
            trace_id:    Request trace ID for log correlation.

        Returns:
            Dict with status, source_name, file_hash, chunks_added, etc.
        """
        ...
        # Duplicate check — same content already in KB
        try:
        ...
        if not chunks:
            return {
                "status": "error",
                "message": f"No usable chunks extracted from '{source_name}'.",
                "code": "EMPTY_CONTENT",
            }
    ...
    # ── Public: search KB (NEW Week 8 Day 2) ─────────────────────────────────
    ...
    def search_kb(
        ...
        """
        Standalone knowledge base search — returns serialisable dicts.
        Used by the /kb/search endpoint (no LangGraph pipeline involved).
        """
        docs = self.retrieve(query=query, top_k=top_k, trace_id=trace_id)
        return [
            {
                "content": doc.page_content,
                "source": doc.metadata.get("source", "unknown"),
                "file_type": doc.metadata.get("file_type", "unknown"),
                "upload_ts": doc.metadata.get("upload_ts", ""),
            }
            for doc in docs
        ]
